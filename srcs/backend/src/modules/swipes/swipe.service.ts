...
//	Feeds the Matches page: every artist who liked ("swiped interested" on)
//	one of the caller's own gigs, newest first. Distinct from
//	getHirerSwipeHistory above, which lists swipes the *hirer* made on
//	candidates — this is the other direction, incoming interest from
//	artists. Optionally narrowed to a single gig; a gigId the caller doesn't
//	own just yields an empty list rather than a 403, same as the `mine`
//	filter on GET /gigs.
export async function handleInterestedArtists(user: AuthenticatedUser, gigId?: string) {
	...
	return swipes
		...
		.map((swipe) => ({
			...
			gig: swipe.gig,
			artist: flattenCategories(swipe.swiper.artistProfile!),
		}));
}
