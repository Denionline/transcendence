...
| GET | `/next` | logged-in | Returns the next eligible candidate. Artist: next `open` gig not yet swiped by the caller. Hirer: `?gigId=...` (required), next artist matching the gig's category not yet reviewed by this hirer; `?onlyInterested=true` narrows this to artists who already liked the gig. `?excludeIds=id1,id2` (optional, either role) additionally skips those ids — for a client holding several not-yet-swiped candidates at once (e.g. a 3-card deck) so each slot gets a distinct one instead of repeating whichever candidate is still first in line |
